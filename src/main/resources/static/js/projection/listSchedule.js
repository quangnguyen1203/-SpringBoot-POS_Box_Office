...
function getMonthSchedule(){
    let schedule_date = App.getCurrentMonthAndYear();
    getAllSchedule(schedule_date);
}
getMonthSchedule();
// <--- edit ---->
...
function searchSchedule(schedule_date) {
    let schedule_month = App.getCurrentMonth();
    if (schedule_date === "") {
        $.ajax({
            type: "GET",
            url: `/schedules/searchSchedule/${schedule_month}`
        }).done(function (schedule) {
            console.log(schedule)
            let content = "";
            for (let i = 0; i < schedule.length; i++) {
